...
export function DataImporter({
  ...
  const addToSchema = (importedData: ImportedData) => {
    const newTable: TableSchema = {
      name: importedData.tableName,
      columns: importedData.columns.map((col, index) => ({
        name: col.name,
        type: col.type,
        primaryKey: index === 0 && col.name.includes("id"),
      })),
    };

    onSchemaChange([...schema, newTable]);
